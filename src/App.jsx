...
// Change this to set your own access code before you deploy.
// NOTE: this is a front-door check only, not real security — the code lives in
...
const DEFAULTS = {
  ...
  discountRatePct: '',
  taxRatePct: '',
  gstCreditRecoverable: false,
};

const NUMERIC_TYPES = ['money', 'percent', 'count', 'number'];
...
function isEmptyField(field, value) {
  return NUMERIC_TYPES.includes(field.type) && value === '';
}
...
const FIELDS_NUMERIC_KEYS = FIELDS.filter((f) => NUMERIC_TYPES.includes(f.type)).map((f) => f.key);
...
function computeModel(rawInp, scenario, mult = {}) {
  const inp = { ...rawInp };
  FIELDS_NUMERIC_KEYS.forEach((k) => { inp[k] = num(rawInp[k]); });
...
function FieldControl({ field, value, onChange }) {
  ...
  return (
    <div className="roi-field">
      ...
      {isMoney && value !== '' && <p className="roi-field__hint">≈ {formatINRCompact(value)}</p>}
      {field.help && <p className="roi-field__help">{field.help}</p>}
    </div>
  );
}
...
function Section({ section, values, onChange, open, onToggle }) {
  ...
  const toReview = sectionFields.filter((f) => isEmptyField(f, values[f.key])).length;
  return (
    <section className="roi-section">
      <button type="button" className="roi-section__header" onClick={onToggle}>
        ...
        {toReview > 0 && <span className="roi-badge">{toReview} to fill</span>}
  ...
  return (
    <div className="roi-gate">
      <style>{`
        ...
        .roi-gate { min-height:100vh; display:flex; align-items:center; justify-content:center; background:linear-gradient(160deg,#0E1B2A,#16283D); padding:20px; font-family:'IBM Plex Sans',system-ui,sans-serif; }
        .roi-gate__card { width:100%; max-width:360px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.14); border-radius:10px; padding:30px 28px; text-align:left; }
        .roi-gate__eyebrow { font-family:'IBM Plex Mono',monospace; font-size:10.5px; letter-spacing:0.14em; text-transform:uppercase; color:#8FB7A8; margin-bottom:10px; }
        .roi-gate__title { font-family:'Space Grotesk',sans-serif; font-size:20px; font-weight:700; color:#F2EFE6; margin:0 0 10px; letter-spacing:-0.01em; }
        .roi-gate__sub { font-size:12.5px; color:#B9C2CC; line-height:1.5; margin:0 0 20px; }
        .roi-gate__input { width:100%; font-family:'IBM Plex Mono',monospace; font-size:14px; padding:10px 12px; border-radius:6px; border:1px solid rgba(255,255,255,0.2); background:rgba(255,255,255,0.06); color:#F2EFE6; margin-bottom:10px; }
        .roi-gate__input:focus { outline:2px solid #1F6F5C; outline-offset:1px; }
        .roi-gate__input::placeholder { color:#7C8894; }
        .roi-gate__error { font-size:12px; color:#E39587; margin:0 0 12px; }
        .roi-gate__btn { width:100%; font-family:'IBM Plex Sans',sans-serif; font-weight:600; font-size:13.5px; padding:10px 12px; border-radius:6px; border:none; background:#1F6F5C; color:#fff; cursor:pointer; }
      ...
      <form className="roi-gate__card" onSubmit={handleSubmit}>
        ...
        <input
          type="password"
          className="roi-gate__input"
          placeholder="Access code"
          value={pw}
          autoFocus
          onChange={(e) => { setPw(e.target.value); setError(false); }}
        />
        {error && <p className="roi-gate__error">Incorrect code — try again.</p>}
        <button type="submit" className="roi-gate__btn">Unlock</button>
      </form>
    </div>
  );
}
...
export default function App() {
  ...
  const emptyCount = FIELDS_NUMERIC_KEYS.filter((k) => inputs[k] === '').length;
  const filledCount = FIELDS_NUMERIC_KEYS.length - emptyCount;

  const capexData = useMemo(() => ([
    { name: 'Machine + GST', value: num(inputs.ultimaExWorks) * (1 + num(inputs.gstPercent) / 100) },
    { name: 'Installation', value: num(inputs.installationLabour) },
    { name: 'Commissioning', value: num(inputs.commissioningExpert) },
    { name: 'Freight & insurance', value: num(inputs.freightInsurance) },
    { name: 'Infrastructure', value: num(inputs.infrastructureCost) },
  ]), [inputs]);
  ...
  const discountPctDisplay = num(inputs.discountRatePct);
  const headline = [
    {
      key: 'npv', label: 'Net present value', sub: `at ${discountPctDisplay}% discount rate`,
    ...
    {
      key: 'irr', label: 'IRR', sub: `vs ${discountPctDisplay}% hurdle rate`,
      value: result.irr !== null ? result.irr * 100 : -20,
      format: result.irr !== null ? `${(result.irr * 100).toFixed(1)}%` : 'not solvable',
      min: -20, max: 100, crossValue: discountPctDisplay, goodDirection: 'high',
    },
    {
  ...
  return (
    ...
    <div className="roi-app">
      <style>{`
        ...
        .roi-empty { text-align:center; padding:34px 24px; margin:0; border-bottom:1px dashed #DED7C6; background:#FCFAF4; }
        .roi-empty__title { font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:16px; color:#1B2430; }
        .roi-empty__sub { font-size:12.5px; color:#8A8375; max-width:440px; margin:8px auto 0; line-height:1.5; }
        ...
        .roi-input::placeholder { color:#BDB49E; font-style:italic; }
      ...
      {filledCount === 0 ? (
        <div className="roi-banner roi-banner--amber">
          ...
          <span>All {FIELDS_NUMERIC_KEYS.length} numbers below are blank — open the sections and start filling them in. The dashboard fills in live as you go.</span>
        </div>
      ) : emptyCount > 0 ? (
        <div className="roi-banner roi-banner--amber">
          <AlertTriangle size={16} />
          <span><strong>{emptyCount} of {FIELDS_NUMERIC_KEYS.length} fields</strong> are still blank — treat the numbers below as partial until every field has a value.</span>
        </div>
      ) : (
        <div className="roi-banner roi-banner--green">
          <CheckCircle2 size={16} />
          <span>Every field has a number in it — these results reflect everything you've entered.</span>
        </div>
      )}

      {filledCount === 0 ? (
        <div className="roi-empty">
          <span className="roi-empty__title">Nothing entered yet</span>
          <p className="roi-empty__sub">Open the sections below and start typing numbers — NPV, IRR, payback, and the charts will appear here automatically as you go.</p>
        </div>
      ) : (
      ...
      <div className="roi-layout">
        <div className="roi-inputs">
          <div className="roi-legend">
            <span><span className="roi-dot is-real" /> A fact you should know or look up (a quote, a spec, your own pipeline numbers)</span>
            <span><span className="roi-dot is-estimate" /> Your own judgment call / assumption</span>
          </div>
          {SECTIONS.map((sec) => (
            <Section
              key={sec.id}
      ...
      <div className="roi-print-report">
        ...
        <table className="rp-table rp-headline">
          <tbody>
            <tr><td>Net present value</td><td>{formatINRCompact(result.npv)}</td><td>at {discountPctDisplay}% discount rate</td></tr>
            <tr><td>IRR</td><td>{result.irr !== null ? `${(result.irr * 100).toFixed(1)}%` : 'not solvable'}</td><td>vs {discountPctDisplay}% hurdle rate</td></tr>
        ...
        <p className="rp-footnote">* your own estimate/assumption. {emptyCount} of {FIELDS_NUMERIC_KEYS.length} fields were still blank when this was generated. Illustrative model, not financial advice.</p>
      </div>
    </div>
    </PasswordGate>
  );
